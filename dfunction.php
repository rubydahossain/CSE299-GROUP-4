...
function signup($data)
{
	...
	$check = database_run("select * from user_doctor where email = :email limit 1",['email'=>$data['email']]);
	if(is_array($check)){
	...
	if(count($errors) == 0){
		...
		$query = "insert into user_doctor (username,email,password,date) values 
...
function login($data)
{
	...
	if(count($errors) == 0){
		...
		$query = "select * from user_doctor where email = :email limit 1";
